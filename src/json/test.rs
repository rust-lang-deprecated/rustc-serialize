...
use json::stack::{StackElement, Stack, SemiPubMethods};
use json::encoder::Encoder;
use json::error::EncoderError;
use json::pretty_json::as_pretty_json;
use json::as_json::as_json;
use json::decoder::decode;
...
fn test_decode_option_none() {
    let s ="{}";
    let obj: OptionData = decode(s).unwrap();
...
fn test_write_enum() {
    let animal = Dog;
    assert_eq!(
        format!("{}", as_json(&animal)),
        "\"Dog\""
    );
    assert_eq!(
        format!("{}", as_pretty_json(&animal)),
        "\"Dog\""
    );
...
macro_rules! check_encoder_for_simple {
    ($value:expr, $expected:expr) => ({
        let s = format!("{}", as_json(&$value));
        assert_eq!(s, $expected);

        let s = format!("{}", as_pretty_json(&$value));
        assert_eq!(s, $expected);
    })
}
...
fn test_decode_identifiers() {
    let v: () = decode("null").unwrap();
    assert_eq!(v, ());

    let v: bool = decode("true").unwrap();
    assert_eq!(v, true);

    let v: bool = decode("false").unwrap();
    assert_eq!(v, false);
}
...
fn test_decode_numbers() {
    let v: f64 = decode("3").unwrap();
    assert_eq!(v, 3.0);

    let v: f64 = decode("3.1").unwrap();
    assert_eq!(v, 3.1);

    let v: f64 = decode("-1.2").unwrap();
    assert_eq!(v, -1.2);

    let v: f64 = decode("0.4").unwrap();
    assert_eq!(v, 0.4);

    let v: f64 = decode("0.4e5").unwrap();
    assert_eq!(v, 0.4e5);

    let v: f64 = decode("0.4e15").unwrap();
    assert_eq!(v, 0.4e15);

    let v: f64 = decode("0.4e-01").unwrap();
    assert_eq!(v, 0.4e-01);

    let v: u64 = decode("0").unwrap();
    assert_eq!(v, 0);

    let v: u64 = decode("18446744073709551615").unwrap();
    assert_eq!(v, u64::MAX);

    let v: i64 = decode("-9223372036854775808").unwrap();
    assert_eq!(v, i64::MIN);

    let v: i64 = decode("9223372036854775807").unwrap();
    assert_eq!(v, i64::MAX);
...
fn test_decode_str() {
    ...
    for &(i, o) in s.iter() {
        let v: string::String = decode(i).unwrap();
        assert_eq!(v, o);
    }
}
...
fn test_decode_array() {
    let v: Vec<()> = decode("[]").unwrap();
    assert_eq!(v, vec![]);

    let v: Vec<()> = decode("[null]").unwrap();
    assert_eq!(v, vec![()]);

    let v: Vec<bool> = decode("[true]").unwrap();
    assert_eq!(v, vec![true]);

    let v: Vec<isize> = decode("[3, 1]").unwrap();
    assert_eq!(v, vec![3, 1]);

    let v: Vec<Vec<usize>> = decode("[[3], [1, 2]]").unwrap();
    assert_eq!(v, vec![vec![3], vec![1, 2]]);
}

#[test]
fn test_decode_tuple() {
    let t: (usize, usize, usize) = decode("[1, 2, 3]").unwrap();
    assert_eq!(t, (1, 2, 3));

    let t: (usize, string::String) = decode("[1, \"two\"]").unwrap();
    assert_eq!(t, (1, "two".to_string()));
}

#[test]
fn test_decode_tuple_malformed_types() {
    assert!(decode::<(usize, string::String)>("[1, 2]").is_err());
}

#[test]
fn test_decode_tuple_malformed_length() {
    assert!(decode::<(usize, usize)>("[1, 2, 3]").is_err());
}
...
fn test_decode_struct_with_nan() {
    ...
    let obj: FloatStruct = decode(s).unwrap();
    assert!(obj.f.is_nan());
...
fn test_decode_option() {
    let value: Option<string::String> = decode("null").unwrap();
    assert_eq!(value, None);

    let value: Option<string::String> = decode("\"jodhpurs\"").unwrap();
    assert_eq!(value, Some("jodhpurs".to_string()));
}

#[test]
fn test_decode_enum() {
    let value: Animal = decode("\"Dog\"").unwrap();
    assert_eq!(value, Dog);

    let s = "{\"variant\":\"Frog\",\"fields\":[\"Henry\",349]}";
    let value: Animal = decode(s).unwrap();
...
fn test_decode_map() {
    let s = "{\"a\": \"Dog\", \"b\": {\"variant\":\"Frog\",\
              \"fields\":[\"Henry\", 349]}}";
    let mut map: BTreeMap<string::String, Animal> = decode(s).unwrap();
...
fn test_encode_hashmap_with_numeric_key() {
    ...
    hm.insert(1, true);
    let json_str = as_pretty_json(&hm).to_string();
...
fn test_prettyencoder_indent_level_param() {
    ...
    for i in 0..4 {
        let printed = as_pretty_json(&json).indent(i as u32)
                            .to_string();
